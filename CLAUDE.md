...
| `lib/github-issues.js` | Creates GitHub issues in the bot's own repo (`github.self_repo`); authenticates via GitHub App (JWT → installation token, cached, auto-refresh) with fallback to `GITHUB_ISSUES_TOKEN` / `GITHUB_TOKEN`; uploads screenshots to `_screenshots/` via Contents API |
