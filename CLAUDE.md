...
| `lib/github-issues.js` | Creates GitHub issues in the bot's own repo (`github.self_repo`); `createIssue({ title, body, labels, imageBase64, imageMimeType })` uploads screenshot to `_screenshots/` via GitHub Contents API and embeds a raw URL in the issue body as a Markdown image |
