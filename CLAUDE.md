...
- Identity templates: `identity/CORE.md` (Layer 1, locked) and `identity/IDENTITY.md` (Layer 2 default) both use `{{USER_NAME}}` and `{{ASSISTANT_NAME}}` placeholders resolved at startup via `resolveIdentityPlaceholders()`
