...
| `lib/settings.js` | Key-value user settings in SQLite `user_settings`; `get(key)`, `set(key,val)`, `getAll()`, `getSystemPromptBlock()` (Layer 4: name/formality/style/character_notes), `getIdentity(default)` (Layer 2: returns DB override or default) |
