...
- **Four-layer system prompt** built by `buildSystemPrompt(nowStr)` in `index.js`: Layer 1 `identity/CORE.md` (locked, cached) → Layer 2 IDENTITY + user settings merged by `settings.getIdentityWithSettings()` (user-configurable; includes preferred name/variants, formality, response style, persona, character_notes; cached) → Layer 3 `lib/skills.js::buildSkillsBlock(tools, skillContexts)` (auto-generated from the real tools list + per-skill behavioral instructions from `lib/skill-context.js`; includes `⚙️` annotations and a self-check meta-instruction; not cached) → Layer 4 timestamp only (not cached). `preferred_name` supports comma-separated variants (e.g. `Вова, Вовик, Вов`) — Claude picks the appropriate one by context; startup hardcoded messages use the first variant. User can change Layer 2 via Mini App Settings → «Личность бота» or by telling Snezhanna (→ `update_my_preferences` with `character_notes`). User can set per-skill instructions via `update_skill_context` tool (conversationally) or `get_skill_context` to view them.
...
- **Long-term searchable archive** (`lib/archive.js`): every owner↔bot turn, every monitored-chat message, and every newly-seen email (full body) is written indefinitely to the SQLite `archive` table with an `archive_fts` FTS5 index alongside it; the `search_archive` tool lets Claude answer "what did we discuss about X a couple months ago" / "find in the correspondence with Y about project Z" by keyword (free text is sanitized into a safe FTS5 `MATCH` expression, each token prefix-matched and OR'd — no embeddings, so recall depends on keyword overlap, not semantic similarity). Gmail's `getMessage()` gained a `{ markRead: false }` option so fetching the full body for archiving doesn't mark the owner's unread email as read. This is a real expansion of what's durably stored (previously conversation history was a 24h-TTL rolling window and monitored-chat messages were wiped nightly) and it rides along in the existing daily DB backup to Google Drive.
...
- **GitHub issue relay:** listens for `/report_issue <JSON>` in a shared Telegram group (`ZHORA_REPORT_CHAT`); parses the JSON, creates a GitHub issue via `GITHUB_TOKEN`, sends confirmation to the group + notification to the owner; Snezhanna instances use this instead of having their own GitHub credentials
...
| `lib/gmail.js` | Gmail API adapter (credentials-based, no file I/O); unified message format; functions: `getMessages`, `getMessage(credentials, messageId, { markRead = true })`, `createDraft`, `sendMessage`, `markAsRead`, `getAttachment` |
...
| `lib/mail-manager.js` | Multi-account email coordinator; `pollAll()` dispatches to gmail/imap adapters, bootstraps new accounts (seeds seen IDs without digest on first poll), categorizes messages via `lib/email-classifier.js` (regex `categorize()` as fallback), detects subprojects; `buildEmailDigest()` formats per-account digest; seen tracking in SQLite `email_seen` table |
| `lib/email-classifier.js` | Claude Haiku one-shot email classifier: one API call per poll batch returns a category (`reply_needed`/`event`/`task`/`update`/`info`) per message; classified messages carry `classified: true` + `needsReply` from the verdict (respected by `briefing.looksLikeReplyRequest`); any failure returns `null` → caller falls back to regex; model overrideable via `email_classifier_model` in config |
...
| `lib/archive.js` | Long-term searchable archive: SQLite `archive` table + `archive_fts` FTS5 index; `addEntry()` (transactional dual-insert, never throws), `search(query, opts)` (source_type/chat_name/project/date filters, ranked by FTS `rank`), `buildMatchQuery()` (sanitizes free text into a safe FTS5 `MATCH` expression — quotes each token to avoid reserved-keyword/syntax collisions, prefix-matches, ORs together); fed by `index.js` (owner_chat), `lib/chat-monitor.js` (monitored_chat), `lib/mail-manager.js` (email, full body); exposed to Claude via the `search_archive` tool |
...
| `lib/release-check.js` | Checks `morshin/snezhanna` GitHub releases for updates; `checkForUpdate()` returns `{ release, summary }` if a version newer than `package.json` is available and not yet notified today; summary is generated by a Claude one-shot in Russian; notification state stored in `user_settings` (`update_notified_tag`, `update_notified_date`) |
| `lib/github-issues.js` | Creates GitHub issues in the bot's own repo (`github.self_repo`); priority: Telegram relay via Zhora group (`ZHORA_REPORT_CHAT`) → HTTP proxy (`REPORT_ISSUE_ENDPOINT`) → GitHub App JWT → `GITHUB_ISSUES_TOKEN` / `GITHUB_TOKEN`; screenshots uploaded to `_screenshots/` via Contents API (direct-auth paths only) |
...
GITHUB_TOKEN             # (optional) PAT for reading milestones/releases; also used by Zhora to create GitHub issues from relay; scopes: public_repo or repo
ZHORA_REPORT_CHAT        # (optional) Telegram group chat ID for issue relay: Snezhanna sends /report_issue → Zhora creates GitHub issue; set same value in all instances + Zhora
REPORT_ISSUE_ENDPOINT    # (optional, legacy) URL of main instance API for proxied issue creation; use ZHORA_REPORT_CHAT instead
REPORT_ISSUE_SECRET      # (optional, legacy) shared Bearer secret for /api/report-issue
...
GITHUB_ISSUES_TOKEN      # (optional) fallback PAT if neither relay, proxy, nor App is configured
