...
Format:
```
📦 Коды призового времени

Файл: codes_28032026.md
Осталось: 47 из 100
```
...
5. Send the `.txt` file to parent via `bot.sendDocument()` with caption: "Импортируй этот файл в Time Boss Cloud. Коды уже сохранены в Максе."

Parent downloads the file from Telegram → imports into TBC manually. Done.

Response after sending the file:
```
✅ Сгенерировано 100 кодов

Файл prize_28032026.txt — загрузи в Time Boss Cloud
Коды сохранены в kids/codes_28032026.md
Предыдущих файлов проверено: 3 (всего известных кодов: 287)
```
...
Inglés: Present Perfect — упражнения стр.45-47
Сыну выдан код: 125374ЩЕКА (+30 мин)
Осталось кодов: 47
```

If no codes left → do NOT send code to son. Instead send to son: "¡Misión completada! Papá te dará tu premio pronto 🎁" and to parent: "🏆 Квест выполнен, но коды закончились! Загрузи новую партию в Time Boss Cloud."
...
```
⚠️ Коды призового времени заканчиваются
...
On parse fail → `Формат: /quest Inglés "описание задания" +30мин`
...
await bot.sendDocument(PARENT_CHAT_ID, Buffer.from(txtContent, 'utf8'), {
  caption: 'Импортируй этот файл в Time Boss Cloud...'
}, {
