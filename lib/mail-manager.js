...
function parseCredentials(account) {
  try {
    return JSON.parse(account.credentials);
  } catch {
    return null;
  }
}
...
async function pollAll() {
  ...
  for (const account of accounts) {
    try {
      ...
      // Categorize
      const categorized = newMessages.map(m => {
        const category = categorize(m, account);
        const result = { ...m, accountId: account.id, accountEmail: account.email, category };
...
module.exports = { pollAll, getMessages, getMessage, createDraft, sendMessage, buildEmailDigest };
