...
function addChat(chat) {
  // Guard: never monitor the owner's own DM with the bot — it would echo the
  // owner's messages back into the digest (and is meaningless anyway).
  const ownerId = (process.env.TELEGRAM_ALLOWED_USER_ID || '').replace(/^@/, '');
  if (ownerId && String(chat.chat_id) === ownerId) {
    return { error: 'Это твой личный чат с ботом — добавлять его в мониторинг нет смысла 😊' };
  }
...
function removeChat(chatId) {
  try {
    const res = db.prepare('DELETE FROM monitored_chats WHERE chat_id = ?').run(chatId);
    chatMap.delete(chatId);
    console.log('[ChatMonitor] Removed chat:', chatId);
    return { ok: true, removed: res.changes > 0 };
  } catch (e) {
