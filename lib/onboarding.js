...
function runIntegrationCheck(msg) {
  ...
  lines.push(`${miniAppOk ? '✅' : '⚠️'} Mini App — ${miniAppOk ? `запущен (порт ${api.PORT})` : 'не запущен'}`);
...
async function sendStep(bot, chatId, step, ctx) {
  const rawName = ctx.name || settings.get('preferred_name') || '';
  ...
  switch (step) {
    ...
    case 'user_gender':
      await bot.sendMessage(chatId,
        `${name ? `${name}, ` : ''}в каком роде к тебе обращаться?`,
        kb([
          [
            { text: '👩 В женском',  callback_data: ob('user_gender', 'female')  },
            { text: '👨 В мужском',  callback_data: ob('user_gender', 'male')    },
          ],
          [
            { text: '🔘 Нейтрально', callback_data: ob('user_gender', 'neutral') },
          ],
        ])
      );
      break;
...
function applyAnswer(step, value, appState) {
  switch (step) {
    case 'user_gender':
      settings.set('user_gender', value);
      break;
    case 'bot_persona': {
      const label = BOT_PERSONA_LABELS[value];
      if (label) settings.set('bot_persona', label);
      const voice = value.startsWith('male_') ? 'onyx' : value === 'female_mature' ? 'shimmer' : 'nova';
...
async function start(bot, chatId, msg, appState, config) {
  ...
  const configuredName = (config && config.user && config.user.name) || '';
  const firstName = configuredName || (msg && msg.from && msg.from.first_name) || '';
  const greeting = firstName ? `Привет, ${firstName}!` : 'Привет!';

  await bot.sendMessage(chatId,
    `${greeting} Меня зовут *${assistantName}* — очень приятно познакомиться 😊\n\n` +
    `Буду помогать разгребать рутину: встречи, почта, задачи, проекты, рабочие чаты, тренировки Strava — и утренний брифинг, чтобы день начинался с ясной головой.\n\n` +
    `Ну и просто — если что-то нужно обдумать или разобраться, я рядом.\n\n` +
    `_Дай секунду — проверю все подключения..._`,
    { parse_mode: 'Markdown' }
  );
  ...
  if (!googleOk) {
    ...
    await bot.sendMessage(chatId,
      `Почти всё готово — осталось подключить Google\\. Это даст доступ к календарю, почте и хранилищу\\.\n\n` +
      `Используй Google\\-аккаунт, созданный специально для этого бота — не свой личный\\.\n\n` +
...
async function handleMessage(bot, msg, appState) {
  ...
  const name = (settings.get('preferred_name') || '').split(',')[0].trim();
