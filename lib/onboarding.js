  ...
  if (!googleOk) {
    ...
    const instructions = isAutoRedirect
      ? `1\\. Нажми кнопку ниже — откроется браузер\n` +
        ...
        `4\\. Произойдёт автоматический редирект — всё готово\\!`
      : `1\\. Нажми кнопку ниже — откроется браузер\n` +
        `2\\. Войди в свой Google\\-аккаунт\n` +
        `3\\. Разреши доступ ко всем запрошенным правам\n` +
        `4\\. Браузер покажет ошибку подключения — это нормально\n` +
        `5\\. Скопируй значение \`code=\` из адресной строки \\(до следующего \`&\`\\)\n` +
        `6\\. Отправь мне: \`/auth КОД\``;
    await bot.sendMessage(chatId,
      `Почти всё готово, но Google ещё не подключён — это нужно для календаря, почты и хранилища\\.\n\n` +
      `*Как подключить:*\n${instructions}`,
      {
