...
async function saveWeekData(weekStr, period, activities, totals) {
  const data = {
    week: weekStr,
    period,
  ...
  await gdrive.writeFile(`fitness/weekly/${weekStr}.json`, JSON.stringify(data, null, 2));

  const summary = generateSummary(weekStr, period, activities, totals);
  await gdrive.writeFile(`fitness/weekly/${weekStr}-summary.md`, summary);
...
async function loadWeekData(weekStr) {
  try {
    const content = await gdrive.readFile(`fitness/weekly/${weekStr}.json`);
    if (!content) return null;
    return JSON.parse(content);
  } catch (err) {
    console.error(`[Strava] Failed to read week data ${weekStr}:`, err.message);
    return null;
  }
}
...
async function syncCurrentWeek() {
  ...
  await saveWeekData(weekStr, period, activities, totals);
...
async function buildDigestFitnessBlock() {
  const now = new Date();
  const currentWeek = getISOWeekString(now);
  const prevWeek = getPrevWeekString(currentWeek);

  const currentData = await loadWeekData(currentWeek);
  if (!currentData) {
    return null;
  }
  ...
  const prevData = prevWeek ? await loadWeekData(prevWeek) : null;
  if (prevData) {
