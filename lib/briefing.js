...
// Classifier verdict wins when present (lib/email-classifier.js sets `classified`);
// heuristic fallback: sender is a real person/org AND subject/snippet contains '?'
function looksLikeReplyRequest(msg) {
  if (!msg) return false;
  if (msg.classified === true) return msg.needsReply === true;
  ...
  const results = await Promise.allSettled([
    (async () => {
      ...
      return events.some(e => {
        ...
        return d.toLocaleDateString('sv-SE', { timeZone: config.timezone }) === todayStr;
      });
    })(),
