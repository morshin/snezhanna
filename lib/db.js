...
// DATABASE_PATH env override — lets tests point at a throwaway DB instead of
// the live one (see test/helpers/temp-db.js).
const DB_PATH = process.env.DATABASE_PATH
  ? path.resolve(path.join(__dirname, '..'), process.env.DATABASE_PATH)
  : path.resolve(path.join(__dirname, '..'), config.database.path);
...
// Self-heal on every startup: the DB holds email_accounts.credentials — tighten
// perms in case the file predates this or was widened by a manual edit.
try {
  fs.chmodSync(DB_PATH, 0o600);
} catch (e) {
  console.error('[db] Could not tighten snezhanna.db permissions:', e.message);
}
...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS archive (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    source_type TEXT NOT NULL CHECK (source_type IN ('owner_chat', 'monitored_chat', 'email')),
    chat_id     INTEGER,
    chat_name   TEXT,
    project     TEXT,
    from_name   TEXT,
  ...
  CREATE VIRTUAL TABLE IF NOT EXISTS archive_fts USING fts5(
    content, subject, chat_name, from_name,
    content='archive', content_rowid='id', tokenize='unicode61'
  );
`);
...
function runMigrations() {
  ...
  const files = fs.readdirSync(MIGRATIONS_DIR)
    .filter(f => f.endsWith('.js') && !/^\d+\.\d+\.\d+/.test(f))
    .sort();
