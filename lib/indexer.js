...
// Legacy indexer: config.yadisk has been removed, marking this module as deprecated
if (!config.yadisk) {
  module.exports = {};
  console.log('[Indexer] Skipped: yadisk config no longer present (migrated to Google Drive)');
  process.exit(0);
}
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      results.push({
        ...
        keywords: JSON.stringify(extractKeywords(entry.name))
      });
    }
  }
}
...
function run() {
  ...
  if (!isIncremental) {
    // Full reindex: truncate and repopulate
    db.prepare('DELETE FROM file_index').run();
  }

  const upsert = db.prepare(`
    INSERT INTO file_index (path, name, extension, size_kb, modified, category, keywords, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))
  ...
  const insertMany = db.transaction((rows) => {
    for (const f of rows) {
      upsert.run(f.path, f.name, f.extension, f.size_kb, f.modified, f.category, f.keywords);
    }
  });

  insertMany(files);
  console.log(`[indexer] Done. Indexed ${files.length} files → SQLite file_index.`);
}
