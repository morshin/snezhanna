...
async function loadHistory() {
  try {
    const content = await gdrive.readFile('workload-history.json');
...
async function saveHistory(history, newEntry) {
  try {
    history.history.push(newEntry);
    if (history.history.length > MAX_HISTORY) {
      history.history = history.history.slice(-MAX_HISTORY);
    }
    await gdrive.writeFile('workload-history.json', JSON.stringify(history, null, 2));
