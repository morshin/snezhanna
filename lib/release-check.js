...
// Extracts the "## ⚙️ После обновления" section from release notes, if present
function extractMigrations(body) {
  if (!body) return null;
  const match = body.match(/##\s*⚙️\s*После обновления\s*\n([\s\S]*?)(?=\n##|$)/i);
...
async function getReleasesSince(currentVersion) {
  ...
  return data
    ...
    .map(r => ({ tag: r.tag_name, url: r.html_url, body: r.body, migrations: extractMigrations(r.body) }));
}
...
async function checkForUpdate() {
  try {
    ...
    if (!shouldNotify(latest.tag)) return null;

    const summary = await summarizeChangelog(releases);
    // Collect migrations from all intermediate releases (oldest→newest), deduplicated
    const allMigrations = releases
      .map(r => r.migrations)
      .filter(Boolean)
      .join('\n\n');
    markNotified(latest.tag);
    return { release: latest, summary, migrations: allMigrations || null };
  } catch (e) {
...
module.exports = { checkForUpdate, getCurrentVersion, getReleasesSince };
