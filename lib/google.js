...
// Self-heal on every startup: tighten perms in case the file predates this or
// was widened by a manual edit. Best-effort — the process owns the file, so
// this needs no elevated privileges.
try {
  if (fs.existsSync(TOKEN_FILE)) fs.chmodSync(TOKEN_FILE, 0o600);
} catch (e) {
  console.error('[Google] Could not tighten token.json permissions:', e.message);
}
...
function getAuth() {
  ...
  // A fresh client is built on every call, so without this, a refreshed
  // access_token is used once in memory and then discarded — every
  // subsequent call after expiry pays another refresh round-trip. Persisting
  // it means later calls read a still-valid token straight from disk.
  auth.on('tokens', (tokens) => {
    try {
      const current = fs.existsSync(TOKEN_FILE) ? JSON.parse(fs.readFileSync(TOKEN_FILE, 'utf8')) : {};
      fs.writeFileSync(TOKEN_FILE, JSON.stringify({ ...current, ...tokens }), { mode: 0o600 });
    } catch (e) {
      console.error('[Google] Failed to persist refreshed token:', e.message);
    }
  });
  return auth;
}
...
function getAuthUrl(state) {
  return makeOAuth2Client().generateAuthUrl({ access_type: 'offline', scope: SCOPES, prompt: 'consent', state });
}

// Accepts either a bare authorization code or a full redirect URL (emergency
// fallback path: /auth <code-or-url>) and returns just the code.
function extractAuthCode(arg) {
  const raw = arg.trim();
  try {
    const u = new URL(raw);
...
async function updateEvent(eventId, updates) {
  ...
  if (updates.startTime) patch.start = { dateTime: updates.startTime, timeZone: config.timezone };
  if (updates.endTime) patch.end = { dateTime: updates.endTime, timeZone: config.timezone };
