...
const TOOLS = [
  ...
  },
  {
    ...
    description: 'Скачать и прочитать вложение из письма Gmail (PDF, XLSX, DOCX). Сначала прочитай письмо через read_gmail_message чтобы узнать attachment_id и filename.',
    input_schema: {
      type: 'object',
      properties: {
        message_id: { type: 'string', description: 'ID письма' },
        attachment_id: { type: 'string', description: 'ID вложения (из поля attachments письма)' },
        filename: { type: 'string', description: 'Имя файла вложения (из поля attachments письма) — нужно для повтора при устаревшем токене' },
        mime_type: { type: 'string', description: 'MIME-тип вложения (из поля attachments письма), например application/pdf' }
      },
  ...
  {
    name: 'list_tasks',
    description: 'Показать задачи из трекера (Яндекс.Диск) и открытые GitHub Issues при настроенном GITHUB_TOKEN — один общий список по Эйзенхауэру. Элементы из GitHub помечены полями source: "github", github_url, github_repo; закрыть issue можно только в GitHub.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Получить открытые GitHub Issues из настроенных репозиториев. Используй когда Вова спрашивает о задачах в GitHub, issues, багах или статусе проектов в GitHub.',
    input_schema: {
      type: 'object',
      properties: {
...
async function executeTool(name, input) {
  switch (name) {
    ...
    case 'read_gmail_attachment': {
      // НЕ вызываем getMessageById до скачивания: Gmail генерирует новые токены
      // при каждом messages.get, и переданный attachment_id стал бы невалидным.
      // Просто пробуем скачать напрямую с токеном от Claude.
      let buf;
      let filename = input.filename || 'unknown';
      let mimeType = input.mime_type || 'application/octet-stream';

      try {
        buf = await google.getAttachment(input.message_id, input.attachment_id);
      } catch (err) {
        if (!err.message || !err.message.includes('Invalid attachment token')) throw err;

        // Токен устарел — перечитываем письмо и ищем вложение по имени файла
        console.warn('[Gmail] Invalid attachment token, перечитываем письмо для свежего токена...');
        const msgRetry = await google.getMessageById(input.message_id);
        if (!msgRetry.attachments.length) throw err;

        // Ищем по имени, если оно передано; иначе берём первое непустое вложение
        ...
        console.warn(`[Gmail] Retry с вложением: ${attRetry.filename} (${attRetry.mimeType})`);
        filename = attRetry.filename;
        mimeType = attRetry.mimeType;
        buf = await google.getAttachment(input.message_id, attRetry.id);
      }
    ...
    case 'list_github_issues': {
      ...
      try {
        const issues = await github.getAllOpenIssues(input.project || null);
        if (input.repo) {
          const filtered = issues.filter(i => i.repo === input.repo);
