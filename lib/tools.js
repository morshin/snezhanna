...
const YADISK_TOOLS = new Set(['search_files', 'read_file', 'read_memory', 'write_memory', 'save_file', 'create_race']);
...
function getAvailableTools() {
  ...
  let custom = google.isAuthorized() ? TOOLS : TOOLS.filter(t => !GOOGLE_TOOLS.has(t.name));

  if (integrations.yandex_disk === false) custom = custom.filter(t => !YADISK_TOOLS.has(t.name));
