...
// Форматирует события Calendar для Claude: заменяет сырые ISO-строки на читаемое local-время
// Без этого Claude видит "2026-03-11T10:00:00+01:00" и может интерпретировать как UTC → ошибка на час+
function formatEventsForClaude(events) {
  return events.map(e => {
    ...
    if (out.start?.dateTime) {
      out.start = {
        ...out.start,
        dateTime_local: new Date(out.start.dateTime).toLocaleString('ru-RU', {
    ...
    if (out.end?.dateTime) {
      out.end = {
        ...out.end,
        dateTime_local: new Date(out.end.dateTime).toLocaleString('ru-RU', {
          hour: '2-digit', minute: '2-digit',
          timeZone: config.timezone
        })
      };
