...
const TOOLS = [
  ...
  {
    name: 'send_email',
    description: 'Отправить письмо. ВАЖНО: устанавливай confirmed=true ТОЛЬКО если пользователь явно сказал "отправь", "send it", "да отправляй" в текущем сообщении. Никогда не устанавливай confirmed=true самостоятельно. Если confirmed=false или не указан — создаёт черновик вместо отправки.',
    input_schema: {
      type: 'object',
      properties: {
        account_id: { type: 'integer', description: 'ID ящика' },
        ...
        body: { type: 'string', description: 'Текст письма' },
        in_reply_to: { type: 'string', description: 'ID письма, на которое отвечаем (опционально)' },
        confirmed: { type: 'boolean', description: 'true — только если пользователь явно сказал отправить в этом сообщении' }
      },
      required: ['account_id', 'to', 'subject', 'body']
    }
  },
...
- Ты дала неверный ответ или облажалась → «Ну я всего лишь код, могу тупить. Хочешь, составим баг-репорт разработчику?»
...
Скриншот прикрепляется автоматически, если пользователь прислал фото в этом же сообщении — специально указывать не нужно. Скрин опционален.`,
    input_schema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'update_bot',
    description: `Обновить бота до последней версии с GitHub.

КОГДА использовать:
- Пользователь просит обновиться / обновить бота / поставить новую версию
- Пользователь спрашивает «есть ли обновления?» — сначала проверь, потом предложи

ОБЯЗАТЕЛЬНО перед вызовом:
1. Сообщи текущую и новую версию (если есть обновление)
2. Предупреди что бот перезапустится на ~30–60 секунд
3. Получи явное подтверждение пользователя («да», «давай», «обнови»)

Если обновлений нет — инструмент вернёт это; скажи пользователю что уже актуальная версия.`,
    input_schema: {
      type: 'object',
      properties: {},
      required: []
...
const GITHUB_TOOLS_SET = new Set(['list_github_milestones', 'create_github_issue']);
  ...
  switch (name) {
    ...
    case 'update_bot': {
      ...
      if (!releases || !releases.length) {
        return { up_to_date: true, version: current, message: `Уже актуальная версия v${current}` };
      }
      ...
      exec(
        ...
        { timeout: 5000 },
        (err) => { if (err) console.error('[UpdateBot] spawn error:', err.message); }
      );
      return { updating: true, from: current, to: latest.tag, message: `Запускаю обновление с v${current} до ${latest.tag}. Бот перезапустится через несколько секунд.` };
    }
