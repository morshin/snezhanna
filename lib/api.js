...
const INSTANCE_DIR = path.join(__dirname, '..');
const INSTANCE_NAME = path.basename(INSTANCE_DIR);
...
function validateInitData(initDataRaw) {
  ...
  try {
    ...
    // Check that the user in initData matches TELEGRAM_ALLOWED_USER_ID
    const allowedRaw = (process.env.TELEGRAM_ALLOWED_USER_ID || '').replace(/^@/, '').toLowerCase();
    if (allowedRaw) {
      ...
      if (userId !== allowedRaw && username !== allowedRaw) return false;
    }
...
function serveStatic(res, filePath) {
  ...
  // path.sep guard: a bare prefix check would let a sibling dir like
  // "mini-app-x" through. Exact match covers MINI_APP_DIR itself.
  if (resolved !== MINI_APP_DIR && !resolved.startsWith(MINI_APP_DIR + path.sep)) {
...
let _bot = null;
let _onGoogleAuthSuccess = null;

function setApiContext(appState, saveStateFn, rescheduleBriefingFn, rescheduleEmailPollFn, bot, onGoogleAuthSuccess) {
  _appState = appState;
  _saveState = saveStateFn;
  _rescheduleBriefing = rescheduleBriefingFn || null;
  _rescheduleEmailPoll = rescheduleEmailPollFn || null;
  _bot = bot || null;
  _onGoogleAuthSuccess = onGoogleAuthSuccess || null;
}
...
function parseRoute(method, pathname) {
  ...
  if (method === 'GET'  && pathname === '/api/system/version') return { handler: 'systemVersion' };
  ...
  // Server-to-server issue reporting (Bearer auth, no initData required)
  if (method === 'POST' && pathname === '/api/report-issue')  return { handler: 'reportIssue', skipInitData: true };
...
async function handleRequest(req, res) {
  ...
  // Shared by every OAuth purpose (main account, per-mailbox linking) — `state`
  // (opaque, single-use, issued by us in oauth-state.js) says which one this is
  // and doubles as the anti-CSRF token.
  if (req.method === 'GET' && pathname === '/auth/google/callback') {
    ...
    const sendHtml = (body) => {
      ...
      res.end(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Google Auth</title></head><body style="font-family:sans-serif;padding:2em">${body}</body></html>`);
    };
    if (error || !code) {
      sendHtml(`<h2>❌ Ошибка авторизации</h2><p>${error || 'Код не получен'}</p>`);
      return;
    }
    const pending = oauthState.consumeAuthState(state);
    if (!pending) {
      sendHtml('<h2>❌ Запрос авторизации не найден или устарел</h2><p>Запроси ссылку авторизации заново в боте и попробуй снова.</p>');
      return;
    }
    try {
      if (pending.purpose === 'main') {
        ...
        sendHtml('<h2>✅ Авторизация прошла успешно!</h2><p>Google подключён. Можно закрыть эту вкладку.</p>');
      } else {
        const account = db.prepare('SELECT id, label, email FROM email_accounts WHERE id = ?').get(pending.accountId);
        const tokens = await google.saveTokenForAccount(code);
        db.prepare("UPDATE email_accounts SET credentials = ?, bootstrapped = 0, updated_at = datetime('now') WHERE id = ?")
          .run(emailCredentials.encodeCredentials(tokens), pending.accountId);
        if (_bot && _appState && _appState.chatId) {
          const label = account ? `«${account.label}» (${account.email})` : `#${pending.accountId}`;
          await _bot.sendMessage(_appState.chatId, `✅ Gmail ${label} подключён!`).catch(() => {});
        }
        sendHtml(`<h2>✅ Ящик подключён!</h2><p>${account ? `«${account.label}» (${account.email})` : ''} готов к работе. Можно закрыть эту вкладку.</p>`);
      }
    } catch (err) {
      sendHtml(`<h2>❌ Ошибка</h2><p>${err.message}</p>`);
    }
    return;
  }
  ...
  // Server-to-server endpoints use Bearer auth instead of Telegram initData
  if (route?.skipInitData) {
    const secret = process.env.REPORT_ISSUE_SECRET;
    ...
    // Mini App routes — require valid Telegram initData
  ...
  try {
    switch (route.handler) {
      ...
      case 'chatsAdd': {
        ...
        const addResult = chatMonitor.addChat({ chat_id: Number(body.chat_id), name: body.name, type: body.type || 'personal', category: body.category || null, project: body.project || null });
        if (addResult?.error) { sendJson(res, 400, { error: addResult.error }); break; }
      ...
      case 'emailAccountsList': {
        const accounts = db.prepare("SELECT id, label, email, type, account_type, enabled, credentials FROM email_accounts ORDER BY id").all().map(a => {
          ...
          const c = emailCredentials.readCredentials(a);
          if (c) {
      ...
      case 'emailAccountsCreate': {
        ...
        const credentials = body.credentials ? emailCredentials.encodeCredentials(body.credentials) : null;
      ...
      case 'emailAccountsPatch': {
        ...
        if (body.credentials !== undefined) {
          ...
          if (body.credentials._keep_password) {
            ...
            const old = emailCredentials.readCredentials(existing);
            if (old && old.password) body.credentials.password = old.password;
          }
          sets.push('credentials = ?'); vals.push(emailCredentials.encodeCredentials(body.credentials));
        }
        if (sets.length === 0) { sendJson(res, 400, { error: 'Nothing to update' }); break; }
        sets.push("updated_at = datetime('now')");
        vals.push(route.id);
      ...
      case 'emailAccountsTest': {
        ...
        if (account.type === 'gmail') {
          const creds = emailCredentials.readCredentials(account);
        ...
        // IMAP test
        const creds = emailCredentials.readCredentials(account);
        if (!creds) { sendJson(res, 200, { ok: false, message: 'Не заданы учётные данные (host, user, password)' }); break; }
        if (!creds.host || !creds.user || !creds.password) { sendJson(res, 200, { ok: false, message: 'Неполные учётные данные — нужны host, user, password' }); break; }
        try {
          const Imap = require('imap');
          const result = await new Promise((resolve, reject) => {
            const conn = new Imap({ user: creds.user, password: creds.password, host: creds.host, port: creds.port || 993, tls: creds.tls !== false, tlsOptions: { rejectUnauthorized: creds.allow_invalid_tls !== true, servername: creds.host }, connTimeout: 10000, authTimeout: 8000 });
      ...
      case 'emailAccountAuthUrl': {
        ...
        try {
          const url = google.getAuthUrl(oauthState.createAuthState('account', account.id));
      ...
      case 'emailAccountAuthCode': {
        ...
        try {
          ...
          db.prepare("UPDATE email_accounts SET credentials = ?, bootstrapped = 0, updated_at = datetime('now') WHERE id = ?")
            .run(emailCredentials.encodeCredentials(tokens), route.id);
          sendJson(res, 200, { ok: true });
        } catch (err) {
      ...
      case 'systemVersion': {
        ...
        const now = Date.now();
        if (versionCache && now - versionCache.ts < 86400000) {
          sendJson(res, 200, { current, ...versionCache.data });
          break;
        }
        try {
          const { execSync } = require('child_process');
          const remoteUrl = execSync('git remote get-url origin', { cwd: path.join(__dirname, '..'), timeout: 3000 }).toString().trim();
          const match = remoteUrl.match(/[:/]([^/:]+\/[^/]+?)(?:\.git)?$/);
          if (!match) throw new Error('no remote');
          ...
          const releaseData = await new Promise((resolve, reject) => {
            const req = https.get({
              ...
              headers: { 'User-Agent': 'snezhanna', ...(token ? { Authorization: `Bearer ${token}` } : {}) },
              timeout: 5000
            }, r => {
              let body = '';
              r.on('data', c => body += c);
          ...
          const latest = releaseData.tag_name ? releaseData.tag_name.replace(/^v/, '') : null;
          const updateAvailable = !!(latest && latest !== current);
          versionCache = { ts: now, data: { latest, updateAvailable } };
          sendJson(res, 200, { current, latest, updateAvailable });
        } catch {
          sendJson(res, 200, { current, latest: null, updateAvailable: false });
        }
        break;
      }
      ...
      case 'systemStatus': {
        ...
        const [active, since, errCount] = await Promise.all([
          sh(`systemctl is-active ${INSTANCE_NAME}`),
          sh(`systemctl show ${INSTANCE_NAME} --property=ActiveEnterTimestamp --value`),
          sh(`journalctl -u ${INSTANCE_NAME} --since "10 min ago" --no-pager -q 2>/dev/null | grep -ic "error\\|fatal\\|uncaught" || echo 0`)
        ]);
        sendJson(res, 200, {
      ...
      case 'systemRestart': {
        const { exec } = require('child_process');
        exec(`sudo systemctl restart ${INSTANCE_NAME}`, { timeout: 10000 }, (err) => {
      ...
      case 'systemStart': {
        const { exec } = require('child_process');
        exec(`sudo systemctl start ${INSTANCE_NAME}`, { timeout: 10000 }, (err) => {
          sendJson(res, 200, { ok: !err, error: err ? err.message : null });
        });
        break;
      }
      ...
      case 'systemUpdate': {
        versionCache = null; // force fresh version check after update
        const { exec } = require('child_process');
        exec(`nohup bash ${INSTANCE_DIR}/scripts/update.sh > /tmp/${INSTANCE_NAME}-update.log 2>&1 &`, { timeout: 5000 }, (err) => {
      ...
      case 'reportIssue': {
        ...
        if (!payload.title || !payload.body) { sendJson(res, 400, { error: 'title and body are required' }); break; }
...
module.exports = { start, stop, setApiContext, isRunning, PORT };
