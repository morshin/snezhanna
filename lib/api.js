  ...
  try {
    switch (route.handler) {
      ...
      case 'emailAccountsList': {
        const accounts = db.prepare("SELECT id, label, email, type, account_type, enabled FROM email_accounts ORDER BY id").all();
      ...
      case 'emailAccountsPatch': {
        const body = await readBody(req);
        const allowed = ['label', 'enabled', 'account_type'];
        const sets = [];
        const vals = [];
        for (const k of allowed) {
          if (body[k] !== undefined) { sets.push(`${k} = ?`); vals.push(body[k]); }
        }
        if (body.credentials !== undefined) { sets.push('credentials = ?'); vals.push(JSON.stringify(body.credentials)); }
        if (sets.length === 0) { sendJson(res, 400, { error: 'Nothing to update' }); break; }
        sets.push("updated_at = datetime('now')");
        vals.push(route.id);
