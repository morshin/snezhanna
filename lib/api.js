  ...
  try {
    switch (route.handler) {
        ...
        const accounts = db.prepare("SELECT id, label, email, type, account_type, enabled, credentials FROM email_accounts ORDER BY id").all().map(a => {
          ...
          if (a.credentials) {
            try {
              ...
              creds_info = { host: c.host || '', port: c.port || 993, user: c.user || '', smtp_host: c.smtp_host || '', smtp_port: c.smtp_port || '' };
            } catch {}
          }
          return { id: a.id, label: a.label, email: a.email, type: a.type, account_type: a.account_type, enabled: a.enabled, creds_info };
        });
      ...
      case 'emailAccountsPatch': {
        const body = await readBody(req);
        const allowed = ['label', 'email', 'enabled', 'account_type'];
        const sets = [];
        const vals = [];
        for (const k of allowed) {
          if (body[k] !== undefined) { sets.push(`${k} = ?`); vals.push(body[k]); }
      ...
      case 'emailAccountsTest': {
        ...
        if (account.type === 'gmail') {
          const ok = google.isAuthorized();
          sendJson(res, 200, { ok, message: ok ? 'Gmail OAuth подключён' : 'Gmail не авторизован — отправь /auth боту' });
          break;
        }
        // IMAP test
        let creds;
