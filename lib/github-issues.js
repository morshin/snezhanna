...
async function sendViaRelay({ title, body, labels }) {
  ...
  try {
    ...
    if (msg.includes('not a member') || msg.includes('kicked') || msg.includes('chat not found') || msg.includes('Forbidden')) {
      return {
        error: `Бот не добавлен в группу-ретранслятор Жоры (ZHORA_REPORT_CHAT=${chatId}). ` +
          `Попроси администратора добавить этого бота в группу — тогда создание issues заработает.`
      };
    }
    throw e;
  }
