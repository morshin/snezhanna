...
function getPublicIssuesToken() {
  return config.github && config.github.public_issues_token;
}

function isConfigured() {
  return isTelegramRelayConfigured() || isProxyConfigured() || isAppConfigured() || !!(process.env.GITHUB_ISSUES_TOKEN || process.env.GITHUB_TOKEN) || !!getPublicIssuesToken();
}
...
async function getBearerToken() {
  ...
  return process.env.GITHUB_ISSUES_TOKEN || process.env.GITHUB_TOKEN || getPublicIssuesToken();
}
...
async function sendViaRelay({ title, body, labels }) {
  ...
  try {
    ...
    const msg = e.message || '';
    if (msg.includes('not a member') || msg.includes('kicked') || msg.includes('chat not found') || msg.includes('Forbidden')) {
      return {
        error:
          'Не могу отправить issue — этот бот не состоит в группе-ретрансляторе Жоры.\n\n' +
          'Что делать:\n' +
          '• Если ты сам создавал этого бота — добавь его в Telegram-группу (ZHORA_REPORT_CHAT) и выдай ему право отправлять сообщения.\n' +
          '• Если это чужой бот — напиши разработчику, он добавит бота в группу.'
      };
    }
    throw e;
  }
  console.log('[GithubIssues] Issue relayed to Zhora via Telegram');
  return { queued: true, message: 'Запрос отправлен Жоре — issue будет создан в GitHub через несколько секунд.' };
}
...
async function createIssue({ title, body, labels, imageBase64, imageMimeType }) {
  if (!isConfigured()) return { error: 'Создание тикетов не настроено на этом инстансе. Оператору нужно добавить GITHUB_TOKEN (или ZHORA_REPORT_CHAT) в файл .env и перезапустить бота.' };
