...
// projects/ and tasks/ removed — data is now in SQLite
const REQUIRED_DIRS = ['index', 'memory', 'fitness', 'fitness/weekly', 'fitness/races', 'drafts', 'digests', 'inbox', 'analytics', 'backups'];
...
function ensureDirs() {
  if (config.integrations && config.integrations.yandex_disk === false) {
    console.log('[yadisk-dirs] Yandex Disk integration disabled, skipping');
...
function saveFile(cacheKey, destPath) {
  if (config.integrations && config.integrations.yandex_disk === false) {
    return { error: 'Yandex Disk integration is disabled for this instance' };
  }
